...
export class EncryptorAES {
  async process(input: string, config: AesConfig): Promise<string> {
    ...
    if (variant === 'aes_gcm') {
      ...
      algoName = "AES-CBC";
      if (!iv || iv.length !== 16) {
        throw new Error(`AES-CBC requires exactly 16 bytes for IV. Got ${iv?.length || 0} bytes.`);
      }
      ...
      algoName = "AES-CTR";
      if (!iv || iv.length !== 16) {
        throw new Error(`AES-CTR requires exactly 16 bytes for the Initial Counter Block (IV). Got ${iv?.length || 0} bytes.`);
      }
      // Domyślna długość licznika dla AES-CTR to 64 bity
      algoParams = { name: "AES-CTR", counter: iv as unknown as BufferSource, length: 64 };
    ...
    try {
      ...
      console.error(`[EncryptorAES] Error during ${mode} with ${variant}:`, error);
      throw new Error(`AES ${mode} failed: ${error.message || error.name || 'Unknown Web Crypto Error'}`);
    }
  }
}
