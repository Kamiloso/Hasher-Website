...
export class EncryptorRSA {
  ...
  async process(input: string, config: RsaConfig): Promise<string> {
    ...
    try {
      ...
      if (variant === 'rsa_oaep') {
        if (mode === 'encrypt') {
          if (!publicKey) throw new Error("Public key (PEM) is required for RSA-OAEP encryption.");
        ...
        if (mode === 'decrypt') {
          if (!privateKey) throw new Error("Private key (PEM) is required for RSA-OAEP decryption.");
      ...
      if (variant === 'rsa_pkcs1') {
        if (mode === 'encrypt') {
          if (!publicKey) throw new Error("Public key (PEM) is required for RSA-PKCS1 encryption.");
          
          const forgePublicKey = forge.pki.publicKeyFromPem(publicKey);
          const utf8Bytes = forge.util.encodeUtf8(input); 
          const encrypted = forgePublicKey.encrypt(utf8Bytes, 'RSAES-PKCS1-V1_5');
          
          return forge.util.bytesToHex(encrypted);
        }

        if (mode === 'decrypt') {
          if (!privateKey) throw new Error("Private key (PEM) is required for RSA-PKCS1 decryption.");
          
          const forgePrivateKey = forge.pki.privateKeyFromPem(privateKey);
          const encryptedBytesString = forge.util.hexToBytes(input);
          const decrypted = forgePrivateKey.decrypt(encryptedBytesString, 'RSAES-PKCS1-V1_5');
      ...
      console.error(`[EncryptorRSA] Error during ${mode} with ${variant}:`, error);
      throw new Error(`RSA ${mode} failed: ${error.message || 'Check your keys and input format.'}`);
    }
  }
}
